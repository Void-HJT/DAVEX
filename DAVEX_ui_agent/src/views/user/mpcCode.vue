  ...
  <div class="form-container">
    ...
    <el-form :model="filePost" label-width="50px">
      <el-form-item prop="description" label="">
        <el-input
          v-model="filePost.description"
          class="inputlength"
          placeholder="（可选）请填写文件描述"
        />
      </el-form-item>
    </el-form>
  </div>
...
const handleTab = (event) => {
  ...
  if (event.shiftKey) {
    ...
    if (code.value.substring(start - 4, start) === tabCharacter) {
      code.value =
        code.value.substring(0, start - 4) + code.value.substring(start)
    ...
    // Tab，增加缩进
    code.value =
      code.value.substring(0, start) + tabCharacter + code.value.substring(end)
...
const loadFile = (event) => {
  ...
  if (file) {
    const reader = new FileReader()
    reader.onload = (e) => {
      if (e.target != null) code.value = e.target.result as string
    }
    reader.readAsText(file)
  }
}
