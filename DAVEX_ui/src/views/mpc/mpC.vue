<template>
  <el-col :span="18" :offset="3">
    ...
    >
      <el-form-item label="模型" prop="mpc">
        <el-select v-model="form.mpc" placeholder="请选择模型">
          <el-option
            v-for="(item, index) in modelPartyArray"
            :key="index"
            :label="item.label"
            :value="item.value"
          />
      ...
      <el-form-item>
        ...
        >
          <div v-for="index in pNnum" :key="index">
            <el-button type="primary" @click="openFileTable(index - 1)">
              选择运算方{{ index }}的文件
            </el-button>
          </div>
          <el-dialog
          ...
          <template #footer>
            <span class="dialog-footer">
              ...
              <el-button type="primary" @click="fileTotask">确认</el-button>
            </span>
          </template>
        </el-dialog>
    ...
    >
      ...
      <template #footer>
        <span class="dialog-footer">
          <el-button type="primary" @click="cleanData">好的</el-button>
        </span>
      </template>
    </el-dialog>
  </el-col>
...
onMounted(async () => {
  ...
  partyArray.value = protocolArray.value.map((item) => ({
    label: item.name,
    value: item.id,
  }))
  ...
  modelPartyArray.value = modelArray.value.map((item) => ({
    label: item.name,
    value: item.id,
  }))
})
...
const submitTask = async () => {
  ...
  if (form.value.pN == '2' && form.value.protocol == '') {
    //当用户选择了参与方数量但没有选择协议时自动为其填充
