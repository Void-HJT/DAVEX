    ...
    <el-main>
      <el-table :data="filterResultData">
        ...
        <el-table-column label="操作">
          <template #default="scope">
            <el-button class="small-default-button" @click="readQueryMethod(scope.row.uid)">
              <el-icon><View /></el-icon> 预览文件
            </el-button>
            <el-button class="small-default-button" @click="fetchQueryMethod(scope.row.uid)">
              <el-icon><Download /></el-icon> 获取文件
            </el-button>
            <el-button class="small-delete-button" @click="deleteQueryMethod(scope.row.uid)">
...
const fetchQueryBody = ref({
  outputId: '',
  applicationId: applicationId
})
const deleteQueryBody = ref({
  outputId: '',
  applicationId: applicationId
})
const readQueryBody = ref({
  outputId: '',
  applicationId: applicationId
})

const getQueryResultMethod = async () => {
  try {
    const res = await getQueryResult(getResultBody.value)
...
const fetchQueryMethod = async (outputId) => {
  try {
    fetchQueryBody.value.outputId = outputId
    const res = await fetchQuery(fetchQueryBody.value)
    if (res.data.code == 1) {
...
const deleteQueryMethod = async (outputId) => {
  try {
    deleteQueryBody.value.outputId = outputId
    const res = await deleteQuery(deleteQueryBody.value)
    if (res.data.code == 1) {
      deleteSuccessMessage.value = res.data.message;
      deleteSuccessVisible.value = true
      await getQueryResultMethod()
    }
    else {
...
const readQueryMethod = async (outputId) => {
  try {
