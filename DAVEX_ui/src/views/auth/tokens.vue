...
const getTokenCacheList = async () => {
  try {
    ...
    if (Array.isArray(res.data)) {
      // 转换 Set<Map.Entry<String, TokenResult>> 到适合表格的数据
      tokensData.value = res.data.map(entry => ({
        keycloakUrl: entry.key, // Map.Entry 的 key
        targetId: entry.value.targetId, // TokenResult 的 targetId
        accessToken: entry.value.accessToken, // TokenResult 的 accessToken
        updateTime: entry.value.updateTime // TokenResult 的 updateTime
      }));
    } else {
