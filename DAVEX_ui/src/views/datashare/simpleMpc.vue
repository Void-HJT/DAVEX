<template>
  ...
  function defaultCompileParameters() {
    return [{
      name: 'party_number',
      limit: {min: 1, max: 3, defaultValue: 2},
      required: true,
      auto: false,
      limitType: 'NUM',
      posORflag: 0,
    ...
    {
      name: 'feature_number',
      limit: {min: 1, max: 10, defaultValue: 5},
      required: true,
      auto: false,
      limitType: 'NUM',
      posORflag: 1,
  ...
  const addcompileRarameter = () => {
    const defaultParams = defaultCompileParameters()
    mpcTaskInfo.value.compileParameters.push(...defaultParams)
    console.log(mpcTaskInfo.value)
  }
  const addruntimeRarameter = () => {
    const defaultParams = defaultRuntimeParameters()
    mpcTaskInfo.value.runtimeParameters.push(...defaultParams)
    console.log(mpcTaskInfo.value)
  }
